...
class GridTest {
    ...
    fun aConvertedGridShouldAlwaysComeBackToAnArray() {
        ...
        val convertedGrid = GridTranscriber.translate(notConvertedGrid)
        val convertedGridRevertToNotConverted = GridTranscriber.translate(convertedGrid)
        assertTrue { notConvertedGrid.contentDeepEquals(convertedGridRevertToNotConverted) }
    }

    @Test
    fun anEmptyGridShouldAlwaysBeConsideredAsNotCompleted() {
        val emptyGrid = Grid(Array(GridConstants.NB_ROWS) {
            Array(GridConstants.NB_COLUMNS) { CellConstants.EMPTY }
        })
    ...
    fun aGridOnlyWithSameValuesShouldAlwaysBeConsideredAsInvalid() {
        val invalidGrid = Grid(Array(GridConstants.NB_ROWS) {
            Array(GridConstants.NB_COLUMNS) { 1 }
        })
    ...
    @Test
    fun aNonCompletedGridShouldBeValidAndCompletedAfterBeingSolved() {
        val notCompletedGrid = Grid(arrayOf(
        ...
        assertTrue { notCompletedGrid.isValid() }
        assertTrue { notCompletedGrid.isCompleted() }
    }
}
